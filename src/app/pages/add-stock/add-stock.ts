...
export class AddStock implements OnInit, OnDestroy {
    ...
    this.stockService
      ...
      .subscribe({
        next: () => {
          ...
          // Tutup dialog & kirim data ke parent
  ...
  onCancel(): void {
    this.dialogRef.close(); // Tutup dialog tanpa mengirim data
  }

  // Tambahkan di dalam class AddStock
  getStockValue(stock: Stock): string {
    return stock.symbol; // mengembalikan symbol untuk selectedStock
  }

  getStockDisplay(stock: Stock): string {
    return `${stock.symbol} - ${stock.name || ''}`; // tampilkan symbol + nama jika ada
  }
}
